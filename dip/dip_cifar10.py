...
cv2.imwrite(os.path.join(output_dir, 'noisy_image.jpg'), im[:, :, ::-1]) 
cv2.imwrite(os.path.join(output_dir, f'rectified_{(14 + 1) * 100}.jpg'), to_image(results[14])[:, :, ::-1])  # RGB to BGR
cv2.imwrite(os.path.join(output_dir, f'rectified_{(29 + 1) * 100}.jpg'), to_image(results[59])[:, :, ::-1])  # RGB to BGR
